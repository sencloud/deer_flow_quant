      ...
      <CardHeader>
        <CardTitle>
          <RainbowText animated={state !== "报告已生成"}>
            {title !== undefined && title !== "" ? title : "深入思考并整理思路"}
          </RainbowText>
        </CardTitle>
      </CardHeader>
      <CardFooter>
...
const GREETINGS = ["酷", "听起来不错", "看起来不错", "很棒", "很棒"];
function PlanCard({
  className,
  message,
  interruptMessage,
  ...
  const handleAccept = useCallback(async () => {
    if (onSendMessage) {
      onSendMessage(
        `${GREETINGS[Math.floor(Math.random() * GREETINGS.length)]}! ${Math.random() > 0.5 ? "让我们开始研究吧" : "让我们开始吧"}`,
        {
